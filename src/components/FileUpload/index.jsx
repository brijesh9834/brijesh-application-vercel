  ...
  return (
    <div className="w-full h-full rounded-md flex flex-col min-h-[fit-content]">
      {isImage ? (
        ...
        <div className="w-full h-full flex justify-center items-center bg-gray-100 flex-shrink-0">{ext}</div>
      )}
...
function FileUpload({
  ...
  const validateAndUpload = (files) => {
    const selectedFiles = Array.from(files).filter((file) => file?.size < maxSize);
  ...
  return (
    <label ref={drop} {...restProps} htmlFor={name} className={`cursor-pointer ${className}`}>
      {files?.length == 0 ? <>{Placeholder ? <Placeholder multiple={allowMultiple} /> : null}</> : null}
      <input
        id={name}
        name={name}
        type="file"
